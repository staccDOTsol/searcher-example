...
async fn build_bundles(
    ...
    let wp_keys = std::fs::read_to_string("wpKeys.json").unwrap();
    let wp_keys: Vec<String> = serde_json::from_str(&wp_keys).unwrap();
    let raydium_clmm_keys = std::fs::read_to_string("raydium_clmmKeys.json").unwrap();
    let raydium_clmm_keys: Vec<String> = serde_json::from_str(&raydium_clmm_keys).unwrap();
    ...
    for packet in pending_tx_notification.transactions.iter() {
            ...
            println!("mempool_tx: {:?}", mempool_tx);
            
            let simulation_result = rpc_client
                .simulate_bundle(&VersionedBundle {
            ...
            for account_key in mempool_tx.message.static_account_keys().iter() {
                // do a futures pool for 
                /*
use amm::raydium::pools::get_raydium_pool;
use amm::raydium::clmm::get_raydium_pool as get_raydium_pool_clmm;
use amm::orca::find_pool;
use amm::orca::legacy_pools::get_legacy_pool; */
// if account key in raydiumKeys.json
// get_raydium_pool
if raydium_keys.contains(&account_key.to_string()) {
/*let raydium_pool = calculate_pool_swap_price(rpc_client, account_key.to_string()).await;
if raydium_pool.is_ok() {
    let raydium_pool = raydium_pool.unwrap();
    println!("raydium price: {:?}", raydium_pool);
}*/
